from aqt.qt import QWidget, QVBoxLayout, QFormLayout, QComboBox, QPushButton, QLabel, QHBoxLayout, QCheckBox, QGroupBox, QScrollArea
...
class ProfileTab(QWidget):
    def __init__(self, parent=None):
        ...
        info = QLabel("Todos os campos são opcionais. Ao salvar, cada campo recebe uma data de atualização própria.")
        ...
        self.form.addRow("País", self.country)
        self.form.addRow("Estado", self.state)
        self.form.addRow("Área principal", self.primary_area)
        ...
        self.form.addRow("Tempo de uso do Anki", self.experience)
        self.form.addRow("Nível autodeclarado", self.level)
        self.form.addRow("Faixa etária", self.age)
    ...
    def remove_secondary(self, combo, row, rm):
        for w in (combo, rm):
            row.removeWidget(w); w.deleteLater()
    ...
    def _refresh_secondary_options(self):
        ...
        for combo, _, _ in self.secondary_boxes:
            ...
            combo.clear()
            combo.addItems([x for x in PRIMARY_AREAS if x != primary or x == ""])
    ...
    def load(self):
        ...
        for val in p.get("secondary_areas", [])[:5]: self.add_secondary_area(val)
        ...
        for plat in p.get("platforms_used", []):
            if plat in self.platform_checks: self.platform_checks[plat].setChecked(True)
        self._country_changed()
        self._render_updated_dates()
    def _render_updated_dates(self):
        u = load_config().get("profile_updated_at", {})
        if not u:
            self.updated_label.setText("Ainda não há datas de atualização registradas para o perfil.")
            return
        lines = ["Última atualização por campo:"] + [f"• {k}: {v}" for k, v in sorted(u.items())]
